...
# Adopted to need of Autorun, UsbEject and CallMonitor by Horst Schmid
#      Copyright (C) 2022...2025 by Horst Schmid
...
logInfoNoEcho() {
  ...
  if [[ "$ll" -le  "$LOGLEVEL" ]]; then
    ...
    if [[ -z "$app_name" ]]; then
      ...
      SCRIPTPATHparsehlp="$( cd -- "$(dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 ; /bin/pwd -P )" # /var/packages/<appName>/target/ui/modules/parse_hlp.sh
...
function cgiLogin() {
  ...
  if [[ ${login_result} != "success" ]]; then
    logInfoNoEcho 3 "Access denied, no login permission"
    return 3
  fi
  ...
  if [[ ${login_success} != "true" ]]; then
    logInfoNoEcho 3 "Access denied, login failed"
    return 4
  fi
  ...
  if [ -z "${user_exist}" ]; then
    logInfoNoEcho 2 "User '${syno_user}' does not exist"
    return 5
  fi
  ...
  if id -G "${syno_user}" | grep -q 101; then
    ...
    logInfoNoEcho 4 "authenticate.cgi: syno_user=$syno_user (is no admin)"
  fi
  return 0
}
...
# logInfoNoEcho 5 "parse_hlp.sh is executed with path '$SCRIPTPATHparsehlp'"
if [[ -f "$appData/config" ]]; then
  if [[ -x "$appData/config" ]]; then
    # logInfoNoEcho 4 "parse_hlp.sh, config file $appData/config found"
    # eval "$(grep "LOGLEVEL=" "$appData/config")" # not secure, code injection may be possible
    # shellcheck source=/dev/null
    source "$appData/config" # not secure, code injection may be possible
    logInfoNoEcho 6 "parse_hlp.sh: config read, e.g. BACKGROUND_COLOR='$BACKGROUND_COLOR' and COLW_ICON='$COLW_ICON'"
  else
    logInfoNoEcho 1 "parse_hlp.sh: Permission wrong for '$appData/config', cound not 'source' this file"
  fi
else
