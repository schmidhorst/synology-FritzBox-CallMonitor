...
echo "Start of install_uifile.sh from $SCRIPTPATHwiz with parameters: '$*'"
...
pwFilePathName="${SCRIPTPATHwiz%%/@*}/@appdata/${SYNOPKG_PKGNAME}/pw"
...
for f1 in $fields; do
  ...
  line=$(grep "^$f1=" "$configFilePathName") # try to get item & value from existing configuration
  ...
  if [[ "${!f1}" == "true" ]] || [[ "${!f1}" == "false" ]]; then
    sed -i -e "s|\"@${f1}@\"|${!f1}|g" "$SYNOPKG_TEMP_LOGFILE" 
  elif [[ "${f1}" == "CCU_PW" ]] || [[ "${f1}" == "DAV_PW" ]]; then
    if [[ -r "$pwFilePathName" ]]; then
      line=$(grep "^$f1=" "$pwFilePathName")  # fetch it from file with defaults
      if [[ -z "$line" ]]; then # no such item in pw file
        hint=" (not set)"
        sed -i -e "s|@${f1}@||g" "$SYNOPKG_TEMP_LOGFILE" # empty entry in wizzard      
      else
        sed -i -e "s|@${f1}@|${!f1}|g" "$SYNOPKG_TEMP_LOGFILE" # ${!f1} should be "*****", put that to wizzard      
      fi
    else
      hint=" (no pw file)"
      sed -i -e "s|@${f1}@||g" "$SYNOPKG_TEMP_LOGFILE" # ${!f1} should be "*****", put that to wizzard      
    fi
  else # normal processing: Replace placeholders by old setting, preserve the quotes:
    sed -i -e "s|@${f1}@|${!f1}|g" "$SYNOPKG_TEMP_LOGFILE" 
  fi
  msg="$msg, $f1='${!f1}'$hint"
done
